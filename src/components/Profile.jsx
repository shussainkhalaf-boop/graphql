...
import {
  ...
  GET_PROJECTS_PASS_FAIL_LATEST_PER_PROJECT,
  GET_LATEST_PROJECTS_WITH_XP,
  GET_PISCINE_GO_XP,
  GET_PISCINE_JS_XP,
  GET_PROJECT_XP,
  GET_EARLIEST_PROGRESS_DATE
...
function Profile() {
  ...
  const commonOpts = { skip: !userId, variables: { userId } };

  const { data: xpdata, loading: xpLoading, error: xpError } = useQuery(GEt_Total_XPInKB, commonOpts);
  const { data: piscineGoXPData, loading: piscineGoXPLoading, error: piscineGoXPError } = useQuery(GET_PISCINE_GO_XP, commonOpts);
  const { data: piscineJsXPData, loading: piscineJsXPLoading, error: piscineJsXPError } = useQuery(GET_PISCINE_JS_XP, commonOpts);
  const { data: projectXPData, loading: projectXPLoading, error: projectXPError } = useQuery(GET_PROJECT_XP, commonOpts);
  const { data: projectsData, loading: projectsLoading, error: projectsError } = useQuery(GET_PROJECTS_WITH_XP, commonOpts);
  const { data: latestProjectsData, loading: latestProjectsLoading, error: latestProjectsError } = useQuery(GET_LATEST_PROJECTS_WITH_XP, commonOpts);
  const { data: passFailData, loading: passFailLoading, error: passFailError } = useQuery(
    GET_PROJECTS_PASS_FAIL_LATEST_PER_PROJECT,
    commonOpts
  );
  const { data: earliestProgressData, loading: earliestProgressLoading, error: earliestProgressError } = useQuery(
    GET_EARLIEST_PROGRESS_DATE,
    commonOpts
  );
  ...
  // Use only latest progress per project (by createdAt desc) and count non-null grades
  const latestPerProject = passFailData?.progress || [];
  const passCount = latestPerProject.filter(p => p.grade !== null && p.grade >= 1).length;
  const failCount = latestPerProject.filter(p => p.grade !== null && p.grade < 1).length;
  ...
  // Program start date: earliest project progress date (fallback to account creation)
  const programStartISO = earliestProgressData?.progress?.[0]?.createdAt || currentUser.createdAt;
  const programStartDate = programStartISO ? new Date(programStartISO).toLocaleDateString() : '—';
  const accountCreatedDate = currentUser.createdAt ? new Date(currentUser.createdAt).toLocaleDateString() : '—';
  ...
  const initials = useMemo(() => {
    ...
    return currentUser.login?.slice(0, 2).toUpperCase() || '';
  }, [currentUser]);
              ...
              <div className="border-t border-gray-200">
                <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4 px-4 py-5">
                  ...
                  <div className="space-y-2 col-span-2 sm:col-span-1">
                    ...
                    <p><span className="font-semibold text-purple-600">Started Program:</span> {programStartDate}</p>
                    <p><span className="font-semibold text-purple-600">Account Created:</span> {accountCreatedDate}</p>
                  </div>
                </dl>
              </div>
            </div>
