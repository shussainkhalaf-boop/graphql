...
function Profile() {
  ...
  if (userLoading || xpLoading || !projectsData || !passFailData || !latestProjectsData || !piscineGoXPData || !piscineJsXPData || !projectXPData) {
    return <div className="text-center text-purple-500 font-bold">Loading...</div>;
  }

  if (userError || xpError) {
  ...
  const startedProgramDate = projects.length > 0 ? new Date(projects[0].createdAt).toLocaleDateString() : "N/A";
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-2 space-y-6">
            ...
            <div className="bg-white shadow-lg rounded-lg overflow-hidden">
              <div className="px-4 py-5 bg-purple-600 text-white">
                <h3 className="text-lg font-medium">Basic Information</h3>
              </div>
              <div className="border-t border-gray-200 px-4 py-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex items-center space-x-4">
                  <div className="h-20 w-20 rounded-full bg-purple-500 flex items-center justify-center text-2xl font-bold text-white">
                    {currentUser.firstName ? `${currentUser.firstName[0]}${currentUser.lastName[0]}` : currentUser.login?.slice(0, 2).toUpperCase()}
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <p><span className="font-semibold text-purple-600">Started Program:</span> {startedProgramDate}</p>
            ...
            <div className="bg-white shadow-lg rounded-lg overflow-hidden">
              <div className="px-4 py-5 bg-purple-600 text-white">
                <h3 className="text-lg font-medium">XP Summary</h3>
              </div>
              <div className="px-4 py-5 grid grid-cols-2 sm:grid-cols-3 gap-4">
                <div className="col-span-2 sm:col-span-3 text-lg font-semibold text-purple-700">
                  Total XP: {formatXP(totalXP)}
                </div>
                <div>
                  <p className="font-semibold text-purple-600">Piscine Go XP</p>
                  <p>{formatXP(piscineGoXPTotal)}</p>
                </div>
                <div>
                  <p className="font-semibold text-purple-600">Piscine JS XP</p>
                  <p>{formatXP(piscineJsXPTotal)}</p>
                </div>
                <div>
                  <p className="font-semibold text-purple-600">Project XP</p>
                  <p>{formatXP(projectXPTotal)}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white shadow-lg rounded-lg overflow-hidden">
            <div className="px-4 py-5 bg-purple-600 text-white">
              <h3 className="text-lg font-medium">Finished Projects</h3>
            </div>
            <div className="h-[400px] overflow-y-auto px-4 py-5 scrollbar-thin scrollbar-thumb-purple-500 scrollbar-track-gray-200">
              {projects.map((project, index) => (
                <div key={project.id} className="mb-4">
                  <div className="flex justify-between">
                    <div>
                      <h4 className="font-semibold">{project.object?.name}</h4>
                      <p className="text-sm text-gray-500">Completed: {new Date(project.createdAt).toLocaleDateString()}</p>
                    </div>
